...
const TpaDashboard = () => {
    ...
    const handleApproval = async (claimId) => {
        try {
            const response = await fetch(`http://localhost:5000/user/update/tpaclaimstatus/${claimId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    tpastatus: 'approved' // Set the status to 'approved'
                })
            })
            .then((data) => {
    ...
    const handleRejection = async (claimId) => {
        try {
            const response = await fetch(`http://localhost:5000/user/update/tpaclaimstatus/${claimId}`, {
                ...
                body: JSON.stringify({
                    tpastatus: 'rejected' // Set the status to 'approved'
                })
            })
            .then((data) => {
                console.log(data, "data submitted");
        ...
        <Container fluid>
            <Row>
                ...
                <Col sm={3} className="sidebar-alt">
                    <div className="py-4 px-3">
                        ...
                        <ul className="nav flex-column">
                            ...
                            <Link to="/soc" className="nav-link">View SOC</Link>
                            </li>
                            <li className="nav-item">
                ...
                <Col sm={9} className="main-content">
                    <div className="py-4 px-3">
                        <h1>Welcome to the TPA Dashboard</h1>
                        <p>Please verify and Approve/Reject the claims.</p>
                    </div>
                </Col>
            </Row>
